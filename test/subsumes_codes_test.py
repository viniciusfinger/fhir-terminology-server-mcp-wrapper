"""Integration tests: MCP `subsumes_codes` against `subsumes_codes_ground_truth.json`."""
...
GROUND_TRUTH_PATH = Path(__file__).resolve().parent / "ground_truth" / "subsumes_codes_ground_truth.json"
...
def _case_id(case: dict[str, Any]) -> str:
    ...
    return f"{inp.get('system', '')}|{inp.get('code_a', '')}|{inp.get('code_b', '')}"
...
async def test_subsumes_codes_matches_ground_truth(case: dict[str, Any]) -> None:
    expected = case["expected_output"]
    inp = case["input"]

    async with Client(mcp) as client:
        result = await client.call_tool("subsumes_codes", inp, raise_on_error=False)
