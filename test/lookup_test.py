"""Integration tests: MCP `lookup_code` against `lookup_ground_truth.json`."""
...
GROUND_TRUTH_PATH = Path(__file__).resolve().parent / "ground_truth" / "lookup_ground_truth.json"
...
async def test_lookup_code_matches_ground_truth(case: dict[str, Any]) -> None:
    expected = case["expected_output"]
    inp = case["input"]

    async with Client(mcp) as client:
        result = await client.call_tool("lookup_code", inp, raise_on_error=False)
